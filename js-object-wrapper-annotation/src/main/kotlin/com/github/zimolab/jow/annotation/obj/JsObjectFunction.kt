 ...
 * @property returnTypeCast String 当返回值类型为非本地类型（Int、Boolean、Double、String、JSObject）时使用的类型转换函数名称。默认为None，代表自动生成一个名称。
 */
...
annotation class JsObjectFunction(
    ...
    val returnTypeCast: String = DEFAULT_RETURN_TYPE_CAST_STRATEGY,
) {
    companion object {
        const val SKIP = false
        const val UNDEFINED_AS_NULL = true
        const val RAISE_EXCEPTION_ON_UNDEFINED = false
        const val DEFAULT_RETURN_TYPE_CAST_STRATEGY = AUTO_DETERMINE
    }
}
